...
# Executor 框架
以上创建线程池的方式，从源码可见，都是调用 ThreadPoolExecutor 类实现的，而该类属于 JUC 中 Executor 框架接口的具体实现。
...
## 从 Executor 接口开始
该接口用 Runnable/Callable 来表示任务，提供了一种标准的方法将任务的提交过程与执行过程解耦，并提供对生命周期的支持和统计信息收集、性能监视等机制。
```java
